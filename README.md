...
Este projeto é uma **aplicação web de arquivo único (Single File Application)** desenvolvida para simular o cálculo do Imposto de Renda Pessoa Física (IRPF) com base nas novas regras sancionadas em novembro de 2025. O foco principal é a validação da isenção para quem ganha até R$ 5.000,00.
...
* **JavaScript (Vanilla):** Lógica de cálculo e manipulação do DOM. Não requer frameworks pesados (como React ou Vue).
...
1.  **Entrada:** Recebe o Salário Bruto.
2.  **Dedução do INSS:** Calcula o INSS de forma progressiva (7.5% a 14%) respeitando o teto do RGPS.
3.  **Resultado Intermediário:** `Base de Cálculo = Salário Bruto - INSS`.
4.  **Cálculo do IR (Nova Regra):**
...
5.  **Saída:** Exibe o Salário Líquido e a Alíquota Efetiva.
...
Não é necessária a instalação de dependências (npm, node, etc).

1.  Baixe o arquivo `index.html`.
2.  Abra-o em qualquer navegador moderno (Chrome, Firefox, Edge, Safari).
3.  **Requisito:** É necessário conexão com a internet para carregar o Tailwind CSS e o FontAwesome (CDN).

🌐 **Acesso Online:** [Clique aqui para acessar](https://capx8.github.io/calculadora_IRPF/)
